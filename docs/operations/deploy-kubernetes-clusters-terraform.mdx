...
- Provision an AKS cluster using Azure Verified Modules
...
Let's deploy an AKS cluster using AVM. For this workshop, we will use the [avm-res-containerservice-managedcluster](https://registry.terraform.io/modules/Azure/avm-res-containerservice-managedcluster/azurerm/0.4.2) resource module, which is the current supported Azure Verified Module for creating AKS clusters.
...
This code sets the module version to `0.4.2` but you should always refer to the Terraform Registry docs to check for the latest stable version.

:::

The example code calls for you to insert the required variables. If you click on the **Inputs** tab, you will see the required variables listed. The minimum required variables are `location`, `name`, and `parent_id` (the resource group ID). You can also see the optional variables that you can use to customize the AKS cluster.

Add the following code to add a new module block with the minimum required variables:
...
This is a minimal example to get started quickly. For more advanced configurations, check out the [additional examples](https://registry.terraform.io/modules/Azure/avm-res-containerservice-managedcluster/azurerm/0.4.2?tab=examples) available in the module documentation, including the [automatic](https://registry.terraform.io/modules/Azure/avm-res-containerservice-managedcluster/azurerm/0.4.2/examples/automatic) and [automatic-private](https://registry.terraform.io/modules/Azure/avm-res-containerservice-managedcluster/azurerm/0.4.2/examples/automatic-private) examples for deploying AKS Automatic clusters.

:::

Let's also add an output block to display the AKS cluster name. Add the following code to your `outputs.tf` file:
...
Within a few minutes, Terraform will create the AKS cluster in Azure. You can verify that the resource was created by checking the Azure portal or by running the following command in the Azure CLI:
...
```text
I'd like to replace the module code in my main.tf file with the equivalent azurerm resources to deploy an AKS cluster using the azurerm provider.
```
...
- **AKS Cluster Creation**: Leveraged the `avm-res-containerservice-managedcluster` resource module to deploy an AKS cluster
