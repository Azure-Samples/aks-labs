...
Let's deploy an AKS cluster using AVM. For this workshop, we will deploy an AKS cluster using the [avm-res-containerservice-managedcluster](https://registry.terraform.io/modules/Azure/avm-res-containerservice-managedcluster/azurerm/0.4.2) resource module, which is the current supported Azure Verified Module for creating AKS clusters.
...
The example code calls for you to insert the required variables. If you click on the **Inputs** tab, you will see the required variables listed. The required variables are `location`, `name`, `parent_id` (the resource group ID), and `default_node_pool`. You can also see the optional variables that you can use to customize the AKS cluster.
...
Within a few minutes, Terraform will create the AKS cluster in Azure. You can verify that the resources were created by checking the Azure portal or by running the following command in the Azure CLI:
