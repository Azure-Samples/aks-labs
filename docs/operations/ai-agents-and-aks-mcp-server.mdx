...
- Configure the AKS MCP Server with different AI agents:
    - [CLI Agent](https://learn.microsoft.com/en-us/azure/aks/cli-agent-for-aks-overview)
...
<Tabs groupId="agents">
    <TabItem value="cli" label="CLI Agent">
        The CLI Agent supports AKS-MCP out of the box, so we only need to install the extension and configure the LLM connection.
        ...
        #### Step 4: Initializing the AKS Agent

        Run the following command to initialize the AKS agent:
        ...
        :::note
        You can also use **Client mode** to run the agent locally since case you don't want to deploy the agent as a pod in your cluster.
        :::
        ...
        Enter value for deployment_name:  (Hint: ensure your deployment name is the same as the model name, e.g., gpt-5) gpt4-1
        Enter value for api_key:
        Enter value for api_base:
        Enter value for api_version:
        ```

        Next, you will be asked to provide the service account name **"aks-mcp-sa"** created in step 4:
        ...
        Do you want to configure managed identity client ID? (Y/n): 12345678-abcd-efgh-ijkl-mnopqrstuvwxzy
        ✅ Using managed identity client ID: 12345678-abcd-efgh-ijkl-mnopqrstuvwxzy
        ```
...
<Tabs groupId="agents">
    <TabItem value="cli" label="CLI Agent">
        Start an interactive session with the CLI agent:
        ```bash
...
<Tabs groupId="agents">
    <TabItem value="cli" label="CLI Agent">
        Now let's use the AI agent to troubleshoot the connectivity issue we just created. Start an interactive session with the CLI agent if you haven't already:
...
<Tabs groupId="agents">
    <TabItem value="cli" label="CLI Agent">
        Now let's use the AI agent to troubleshoot the application issue. Start an interactive session with the CLI agent if you haven't already:
        ...
        ```bash
        Can you help me observe DNS resolution issues in in real-time for 30 seconds? I'm seeing slow DNS requests or errors.
        ```
...
- **Configured AI agents** (CLI Agent, etc.) with the AKS-MCP Server
...
- [AKS Agent](https://learn.microsoft.com/en-us/azure/aks/cli-agent-for-aks-overview)
